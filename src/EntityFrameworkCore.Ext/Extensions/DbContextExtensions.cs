using EntityFrameworkCore.Ext.Attributes;
...
public static class DbContextExtensions
{
  ...
  public static void EnsureAutoHistory<TAutoHistory>(this DbContext dbContext, Func<TAutoHistory> historyFactory) where TAutoHistory : AutoHistory {
    EnsureAutoHistory(dbContext, historyFactory, EntityState.Modified, EntityState.Deleted);
  }
  ...
  public static void EnsureAutoHistory<TAutoHistory>(this DbContext dbContext, Func<TAutoHistory> historyFactory, params TrackedEntity[] trackedEntities) where TAutoHistory : AutoHistory {
    if (dbContext == null) throw new ArgumentNullException(nameof(dbContext), $"{nameof(dbContext)} cannot be null.");

    if (historyFactory == null) throw new ArgumentNullException(nameof(historyFactory), $"{nameof(historyFactory)} cannot be null.");

    if (trackedEntities == null) throw new ArgumentNullException(nameof(trackedEntities), $"{nameof(trackedEntities)} cannot be null.");
    ...
    foreach (var trackedEntity in trackedEntities.Where(entity => entity != null)) {
      ...
      if (autoHistory != null) dbContext.Add(autoHistory);
    }
  }
  ...
  public static TrackedEntity[] DetectChanges(this DbContext dbContext, params EntityState[] entityStates) {
    ...
    if (entityStates == null) throw new ArgumentNullException(nameof(entityStates), $"{nameof(entityStates)} cannot be null.");

    // .ToArray() is needed here for excluding the AutoHistory model
    var trackedEntities = dbContext.ChangeTracker.Entries()
                                   .Where(entry => entry.Metadata.ClrType.GetCustomAttributes(typeof(ExcludeFromHistoryAttribute), true).Length == 0)

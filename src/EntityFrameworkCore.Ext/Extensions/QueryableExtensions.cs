...
using EntityFrameworkCore.Ext.Abstractions;
...
internal static class QueryableExtensions
{
  public static IQueryable<T> Include<T>(this IQueryable<T> source, IList<Func<IQueryable<T>, IIncludableQueryable<T, object>>> includes) where T : class {
    if (!(includes?.Any() ?? false)) return source;

    return includes.Aggregate(source, (queryable, include) => include == null
                                                                ? queryable
                                                                : include(queryable));
  }
  ...
  public static IQueryable<T> Top<T>(this IQueryable<T> source, ITopping topping) where T : class {
    if (!(topping?.TopRows > 0)) return source;
      ...
      if (sorting.SortDirection == SortDirection.Ascending) {
        if (!orderedQueryable) {
          if (!string.IsNullOrWhiteSpace(sorting.FieldName)) {
            ...
            if (success) orderedQueryable = true;
          }
          ...
          else if (sorting.KeySelector != null) source = ((IOrderedQueryable<T>)source).ThenBy(sorting.KeySelector);
        }
      }
        ...
        if (!orderedQueryable) {
          ...
          else if (sorting.KeySelector != null) source = ((IOrderedQueryable<T>)source).ThenByDescending(sorting.KeySelector);
        }
      }
  ...
  public static IOrderedQueryable<T> OrderBy<T>(this IQueryable<T> source, string fieldName, out bool success) where T : class {
    ...
    var queryable = (expression == null
                       ? source
                       : source.Provider.CreateQuery<T>(expression)) as IOrderedQueryable<T>;
